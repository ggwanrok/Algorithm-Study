...
void func(){
    ...
    // //막줄은 해당 열에 만약 도달한다면 최종 도착까지 얼마의 코스트가 소모될지를 저장.
    // dp[n-1][m-1][0] = dp[n-1][m-1][1] = area[n-1][m-1];
    // for(int i=m-2; i>=0; i--){
    //     dp[n-1][i][0] = dp[n-1][i][1] = area[n-1][i] + dp[n-1][i+1][0];
    // }

    // //n-2행까지의 최대 정보와 n-1행의 정보를 종합적으로 판단하여 최댓값에 대한 저장을 하면 될듯.
    // for(int i=0; i<m; i++){
    //     res = max(res, max(dp[n-2][i][0], dp[n-2][i][1]) + dp[n-1][i][0]);
    // }
}
